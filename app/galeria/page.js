...
export default function GaleriaPage() {
  ...
  const photos = active.kind === 'photos' ? (PHOTOS[active.key] ?? []) : []
    ...
    <PageShell width="wide">
      ...
      <div className="-mx-5 overflow-x-auto px-5 no-scrollbar sm:mx-0 sm:px-0">
        <div className="inline-flex min-w-max gap-1 rounded-2xl border border-hairline bg-surface-2 p-1">
          {SECTIONS.map(s => {
            const n = s.kind === 'photos' ? (PHOTOS[s.key] ?? []).length : INTERVIEWS.length
            return (
              <button
                key={s.key}
  ...
  if (photos.length === 0) {
    return (
      <EmptyState
        ...
        description={
          year === '2026'
            ? 'Las imágenes se irán publicando durante el torneo, del 6 al 9 de agosto.'
