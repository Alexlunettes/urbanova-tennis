 ...
 * winners.
 *
 * SQUADS are formed only at that point. The four survivors of every division
 * are ranked, and squads are built rank against rank: the best remaining pair
 * of each division forms the first squad, the second-best of each the second,
 * and so on. From the semifinals on it is squad versus squad over four
 * matches, one per division, and the majority advances.
 *
...
/**
 * Proposes the four squads from the survivors of each division.
 *
 * Rank against rank: the best remaining pair of every division makes up the
 * first squad, and so on down. Returns four slots regardless of how many
 * divisions have finished, so the admin panel and the public bracket can both
 * show the shape before it is complete.
 *
 * @param {Record<number, {survivors: Array}>} byDivision keyed by level
 */
export function proposeSquads(byDivision = {}) {
  const size = 4
  const ready = LEVELS.every(l => byDivision[l]?.complete)

  const squads = Array.from({ length: size }, (_, i) => {
    const members = {}
    for (const level of LEVELS) {
      members[level] = byDivision[level]?.survivors?.[i] ?? null
    }
    return {
      seed: i + 1,
      name: `Escuadra ${i + 1}`,
