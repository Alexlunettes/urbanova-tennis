...
import { QUARTERFINAL_ADVANCEMENT } from './tournament-data.js'
 ...
 *
 * The final fills itself in. As soon as both semifinals have a winner, the
 * final's two teams are PROJECTED from them rather than waiting for the
 * organisers to press anything — the same derive-don't-store rule the rest of
 * the bracket follows, so correcting a semifinal score immediately changes who
 * is shown in the final. The projection is marked `projected` and carries no
 * `squad1_id`, which is how the admin panel still knows the final's own matches
 * have yet to be created.
 */
export function buildBracket(encounters = [], matches = []) {
  const byEncounter = groupMatchesByEncounter(matches)

  const built = encounters
    .slice()
  ...
  const semis = built.filter(b => b.round === 'semifinal')
  const final = built.find(b => b.round === 'final')

  if (final && !final.squad1 && !final.squad2 && semis.length === 2) {
    const winners = semis.map(advancingSquad)
    // Both semifinals must be settled; one decided tie is not a final.
    if (winners.every(Boolean)) {
      final.squad1   = winners[0]
      final.squad2   = winners[1]
      final.projected = true
    }
  }
...
/**
 * Which pair carries on from a quarterfinal.
 *
 * Normally the winner, but QUARTERFINAL_ADVANCEMENT can redirect a specific
 * tie — see the note there. The recorded result is never consulted for
 * anything except identifying the two sides, so the match itself keeps
 * reporting the winner it actually had.
 *
 * @returns {{team_id: string, team_name: string, viaOverride: boolean}|null}
 */
export function advancingFromQuarterfinal(match) {
  if (!match?.completed || !match.winner_id) return null

  const winnerIsTeam1 = match.winner_id === match.team1_id
  const winner = {
    team_id:   match.winner_id,
    team_name: (winnerIsTeam1 ? match.team1?.name : match.team2?.name) ?? '—',
  }
  const loser = {
    team_id:   winnerIsTeam1 ? match.team2_id : match.team1_id,
    team_name: (winnerIsTeam1 ? match.team2?.name : match.team1?.name) ?? '—',
  }

  // Both sides must match the entry, so a re-drawn slot cannot inherit a rule
  // written for a different pair.
  const rule = QUARTERFINAL_ADVANCEMENT.find(r =>
    r.level === match.level &&
    r.slot  === match.slot &&
    r.winner   === winner.team_name &&
    r.advances === loser.team_name)

  return rule
    ? { ...loser,  viaOverride: true }
    : { ...winner, viaOverride: false }
}
...
export function rankSurvivorsForDivision(level, quarterfinals = [], groupStandings = []) {
  ...
  for (const qf of quarterfinals) {
    const advancing = advancingFromQuarterfinal(qf)
    if (!advancing) continue
    winners.push({
      team_id:     advancing.team_id,
      team_name:   advancing.team_name,
      viaBye:      false,
      // Always the WINNER's performance, even when an advancement entry sends
      // the other pair through: the entry hands over that quarterfinal's place
      // in the division whole, seeding included, so the slot behaves exactly as
      // it would have. For every ordinary tie the two are the same pair anyway.
